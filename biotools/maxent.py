...
def run_maxent(environmentallayers, samplesfile, outputdirectory, **kwargs):
    """
    Args:
        ...
        Options of basicRun:
            `redoifexists`: boolean, default `True`
    ...
    """
    kwargs.setdefault("redoifexists", True)
    ...
    kwargs.setdefault("appendtoresultsfile", False)
    kwargs.setdefault("writebackgroundpredictions", False)

    with importlib.resources.path("lib", "maxent.jar") as path:
        command = [
    ...
    output_dir = pathlib.Path(outputdirectory)
